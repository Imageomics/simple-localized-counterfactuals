"""
Adapted from https://github.com/guillaumejs2403/ACE/blob/main/eval_utils/cout_metrics.py
COUT metric extrated from: https://github.com/khorrams/c3lt
"""
...
@torch.no_grad()
def compute_cout(label, target, classifier, loader, binary):
    """
    ...
    with torch.no_grad():
        for i, (img, cf) in enumerate(tqdm(loader)):
            img = img.to(dtype=torch.float).cuda()
