...
from slc.constants import DatasetNames, get_cheetah_cougar_label_map
...
def run():
    ...
    dset = ImageFolderDatasetInterface(
        root=get_dataset_root(DatasetNames.CHEETAH_COUGAR)
    )
    cheetah_cougar = dset.get_torch_dataset(phase="train", with_paths=True)
    label_map = get_cheetah_cougar_label_map()

    tmp_dir = Path("/local/scratch2/carlyn.1/rcvcf/tmp/")
    cf_dir = tmp_dir / "tmp_cc_results"
    ...
    for IDX, (image, label, p) in tqdm(
        enumerate(cheetah_cougar),
        total=len(cheetah_cougar),
        desc="Cheetah-Cougar Experiment",
        colour="#3f618d",
        position=1,
    ):  # type: ignore
