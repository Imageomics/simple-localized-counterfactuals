...
from slc.constants import DatasetNames, get_egyptian_persian_label_map
...
def run():
    ...
    dset = ImageFolderDatasetInterface(
        root=get_dataset_root(DatasetNames.EGYPTIAN_PERSIAN)
    )
    egyptian_persian = dset.get_torch_dataset(phase="train", with_paths=True)
    label_map = get_egyptian_persian_label_map()

    tmp_dir = Path("/local/scratch2/carlyn.1/rcvcf/tmp/")
    cf_dir = tmp_dir / "tmp_ep_results"
    ...
    for IDX, (image, label, p) in tqdm(
        enumerate(egyptian_persian),
        total=len(egyptian_persian),
        desc="Egyptian-Persian Experiment",
        colour="#3f618d",
        position=1,
    ):  # type: ignore
