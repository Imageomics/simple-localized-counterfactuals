...
class CustomMaskImagenetDataset(Dataset):
    ...
    def __init__(
        ...
        self.dataset = load_dataset(
            "imagenet-1k", split=self.split, trust_remote_code=True
        )
        ...
        self.data["image"] = self.data["path"].apply(
            lambda x: Image.open(x + "/image.png")
        )
        self.data["mask"] = self.data["path"].apply(
            lambda x: Image.open(x + "/image_mask.png")
        )
    ...
    def __getitem__(self, index):
        ...
        return (
            ...
            label,
            pred_label.item(),
            self._get_mask_for_index(index),
        )
    ...
    def _get_mask_for_index(self, index):
        ...
        while min(*mask.size) >= 2 * self.img_size:
            mask = mask.resize(tuple(x // 2 for x in mask.size), resample=Image.BOX)
        ...
        mask_arr = mask_arr[
            crop_y : crop_y + self.img_size, crop_x : crop_x + self.img_size
        ]
    ...
    def filter_data(self, filter_id):
        if filter_id is not None:
            ...
            joint_filtered = joint_filtered[
                joint_filtered.index.isin(self.data["idx"].values)
            ]
