...
class ImageNetDataset(Dataset):
    path_hier_json = "/local/scratch2/carlyn.1/model_weights/rcsb/data/metadata/imagenet/wordnet_hierarchy.json"
    ...
    def __init__(
        ...
        # self.data = load_dataset("imagefolder", data_dir=path_data)
        ...
        for root, dirs, files in Path(path_data).walk():
            for file in files:
                if not file.endswith(".JPEG"):
                    continue
                file_path = Path(root) / file
                cls_name = root.parts[-1]
                data_dict["image"].append(file_path.as_posix())
                data_dict["class_name"].append(cls_name)

        all_class_names = sorted(list(set(data_dict["class_name"])))
        class_name_to_idx = {name: idx for idx, name in enumerate(all_class_names)}
        data_dict["label"] = [
            class_name_to_idx[name] for name in data_dict["class_name"]
        ]
        ...
        self.predictions = (
            pd.read_csv(path_predictions, index_col="idx") if path_predictions else None
        )
    ...
    def filter_data(self, filter_id, n_skip):
        if filter_id is not None:
            ...
            print(gt_labels, self.predictions)
            ...
            if self.predictions is None or (
                self.predictions[gt_labels.gt_label == filter_id]["pred_label"]
                .isna()
                .any()
            ):
                log.error(
                    f"Filtering is not possible without predictions for label: {filter_id}"
                )
            ...
            # joint_filtered = joint[
            #    (joint.pred_label == filter_id) & (joint.pred_label == joint.gt_label)
            # ]

            # TODO: restore the above line for correct predictions filtering
            joint_filtered = joint[
                (joint.gt_label == filter_id)  # & (joint.pred_label == joint.gt_label)
            ]
            ...
            print("Filtered length", len(filtered_idx))
        else:
            ...
            if config.exp.guide_id is not None:
                ...
                def _get_path_to_label(root, target_value, path=[]):
                    ...
                    for child in root["children"]:
                        leaf, leaf_path = _get_path_to_label(
                            child, target_value, path + [root["name"]]
                        )
                ...
                def _get_neighborhood_labels(hier, path_to_label):
                    ...
                    try:
                        root_list = [
                            e for e in hier["children"] if path_to_label[0] == e["name"]
                        ]
                    except:
                        import pdb
                ...
                while n_neighs == 1:
                    if len(path_to_label[: (-config.exp.guide_depth - depth_inc)]) == 0:
                        ...
                        neigh_labels = _get_neighborhood_labels(
                            self.hier, path_to_label[:1]
                        )
                        break
                    neigh_labels = _get_neighborhood_labels(
                        self.hier,
                        path_to_label[: (-config.exp.guide_depth - depth_inc)],
                    )
                    depth_inc += 1
