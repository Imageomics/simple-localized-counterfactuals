...
class SigLIPFeatureExtractor(BaseFeatureExtractorInterface):
    def __init__(
        self,
        hf_path: str,
    ):
        ...
        self.image_encoder = SiglipVisionModel.from_pretrained(hf_path)
        self.hidden_size = self.image_encoder.config.hidden_size
        self.image_encoder = self.image_encoder.vision_model
        self.image_processor = AutoProcessor.from_pretrained(hf_path)
    ...
    def forward(self, x, **kwargs):
        ...
        feats = torch.mean(sequence_output, dim=1)
        return feats


class SigLIPRecognitionModel(BaseRecognitionModelInterface):
